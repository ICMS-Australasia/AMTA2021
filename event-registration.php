<?php
$title = "Event Registration";
include('include/header.php');
?>

<p>The 46th Australian Music Therapy Association National Virtual Conference 2020 (AMTA 2020 Virtual) is open for online registration. Please read the registration information carefully before beginning.</p>

<a href="https://icmsaust.eventsair.com/australian-music-therapy-association-46th-national-conference-pds/amtav" class="button" target="_blank">Click here to register</a>

<h2>Virtual registration fees</h2>
<p>All fees are quoted in Australian dollars ($AUD) and are inclusive of 10% GST (Goods and Services Tax).</p>

<table class="rego-table" cellpadding="10" cellspacing="10" border="0">
    <tr align="center">
        <th></th>
        <th><b>Virtual Professional Development Seminar Registration</b> <br>
            <em>Saturday 7 November 2020</em></th>
    </tr>
    <tr>
        <td>PDS Registration</td>
        <td align="center">$110 AUD</td>
    </tr>
    <tr>
        <td>PDS First Nations Registration</td>
        <td align="center">$0</td>
    </tr>
    <tr align="center">
        <th></th>
        <th><b>Virtual Conference Registration</b> <br>
            <em>Sunday 8 November 2020 - Sunday 15 November</em></th>
    </tr>

    <tr>
        <td colspan="2"><b>Member Registrations</b></td>
    </tr>
    <tr>
        <td>Full conference registration member</td>
        <td align="center">$300 AUD</td>
    </tr>
    <tr>
        <td>3 x Session registration member*</td>
        <td align="center">$160 AUD</td>
    </tr>
    <tr>
        <td>1 x Session registration member*</td>
        <td align="center">$60 AUD</td>
    </tr>
    <tr align="center">
        <td colspan="2"><em>*Session/s will require selection via the online registration form</em></td>
    </tr>

    <tr>
        <td colspan="2"><b>Non-Member Registrations</b></td>
    </tr>
    <tr>
        <td>Full conference registration non-member</td>
        <td align="center">$350 AUD</td>
    </tr>
    <tr>
        <td>3 x Session registration non-member*</td>
        <td align="center">$190 AUD</td>
    </tr>
    <tr>
        <td>1 x Session registration non-member*</td>
        <td align="center">$75 AUD</td>
    </tr>
    <tr align="center">
        <td colspan="2"><em>*Session/s will require selection via the online registration form</em></td>
    </tr>

    <tr>
        <td colspan="2"><b>Student Registrations </b></td>
    </tr>
    <tr>
        <td>Full conference registration student</td>
        <td align="center">$150 AUD</td>
    </tr>
    <tr>
        <td>3 x Session registration student*</td>
        <td align="center">$80 AUD</td>
    </tr>
    <tr>
        <td>1 x Session registration student*</td>
        <td align="center">$30 AUD</td>
    </tr>
    <tr align="center">
        <td colspan="2"><em>*Session/s will require selection via the online registration form</em></td>
    </tr>

    <tr>
        <td colspan="2"><b>First Nations Registrations</b></td>
    </tr>
    <tr>
        <td>Full conference registration first nations</td>
        <td align="center">$0</td>
    </tr>
    <tr>
        <td>3 x Session registration first nations*</td>
        <td align="center">$0</td>
    </tr>
    <tr>
        <td>1 x Session registration first nations*</td>
        <td align="center">$0</td>
    </tr>
    <tr align="center">
        <td colspan="2"><em>*Session/s will require selection via the online registration form</em></td>
    </tr>

    <tr>
        <td colspan="2"><b>Person with a disability Registrations</b></td>
    </tr>
    <tr>
        <td>Full conference registration Person with a disability</td>
        <td align="center">$150 AUD</td>
    </tr>
    <tr>
        <td>3 x Session registration Person with a disability*</td>
        <td align="center">$80 AUD</td>
    </tr>
    <tr>
        <td>1 x Session registration Person with a disability*</td>
        <td align="center">$30 AUD</td>
    </tr>
</table>

<h2>Entitlements</h2>
<h3>Full Conference Registration</h3>
<p>Full registration entitlements include:</p>
<ul>
    <li>Unique attendee password-protected access to the AMTA 2020 virtual conference platform</li>
    <li>All program content delivered live</li>
    <li>Ability to engage with presenters and other attendees during live presentations through a discussion channel & live Q&A</li>
    <li>Ability to take notes during sessions and export to email</li>
    <li>Access to view all presentations on-demand, post live presentation</li>
    <li>Access to view all presentation abstracts and speaker information</li>
    <li>Access to view the AMTA PechaKucha Gallery</li>
    <li>Access to view the AMTA virtual exhibition and opportunity to engage with key sponsors and exhibitors</li>
    <li>Opportunity to meet other attendees, exchange contact details and set meetings via the AMTA 2020 meeting hub</li>
    <li>Approx. 18.25 CPD hours</li>
    <li>Certificate of CPD</li>
</ul>
<h3>Session Registration</h3>
<p>1 or 3 session registration entitlements include:</p>
<ul>
    <li>Unique attendee password-protected access to the AMTA 2020 virtual conference platform</li>
    <li>Selected session content delivered live</li>
    <li>Ability to engage with presenters and other attendees during selected presentations through a discussion channel & live Q&A</li>
    <li>Ability to take notes during selected sessions and export to email</li>
    <li>Access to view selected session presentations on-demand, post live presentation</li>
    <li>Access to view selected session presentation abstracts and speaker information</li>
    <li>Access to view the AMTA PechaKucha Gallery</li>
    <li>Access to view the AMTA virtual exhibition and opportunity to engage with key sponsors and exhibitors</li>
    <li>Opportunity to meet other attendees, exchange contact details and set meetings via the AMTA 2020 meeting hub</li>
    <li>Between 1 – 10.5 CPD hours depending on session amount and session length chosen.</li>
    <li>Certificate of CPD</li>
</ul>
<h3>PDS Registration</h3>
<p>PDS registration entitlements include:</p>
<ul>
    <li>Unique attendee password-protected access to the AMTA 2020 virtual conference platform</li>
    <li>All PDS program content delivered live</li>
    <li>Ability to engage with PDS presenters and other attendees during live presentations through a discussion channel & live Q&A</li>
    <li>Ability to take notes during PDS sessions and export to email</li>
    <li>Access to view all PDS presentations on-demand, post live presentation</li>
    <li>Access to view all PDS presentation abstracts and speaker information</li>
    <li>Approx. 6 CPD hours</li>
    <li>Certificate of CPD</li>
</ul>
<h2>Method of Payment</h2>
<p>Registration can only be confirmed once payment is received.</p>
<p>Payment can be made by Credit card: Visa, MasterCard and American Express are accepted. If you are making a credit card payment from a country outside Australia, for security reasons you must first advise your banking institution of the impending charge and authorise them to allow the charge to Australia to go through. Please note that charges to your credit card will appear as being from “ICMS Australasia Pty Ltd” on your bank statement.</p>
<h2>Payment deadlines</h2>
<p>All registrations must be paid in full by Friday 23 October 2020.</p>
<h3>Registration cancellation & refund policy</h3>
<p>Once your registration has been completed, there will be no refunds available. The Conference content will remain on-line and active for three months after the Conference dates and remain accessible for registered delegates to view at any time during this period, hence providing full value to any registered parties.  The Conference Secretariat will not provide any refunds.</p>
<p>Your registration may be transferred to another person by 25 October 2020. The Conference Secretariat must be advised in writing of any alterations or transfers by this date.</p>
<h2>Disclaimer</h2>
<p>All best endeavours will be made to present the program as printed on this website. The Conference and its agents reserve the right to alter without prior notice, any of the arrangements, timetables, plans or other items relating to the meeting, for any cause beyond its reasonable control. The Organising Committee and the Conference Organisers are not liable for any loss or inconvenience caused as a result of such alteration. In the event of unforeseen circumstances, the Organising Committee and the Conference Organisers do not accept responsibility for loss of monies caused by delays. In registering for this conference relevant details (name/country) will be incorporated into a delegate list for the benefit of all delegates, and also may be made available (if necessary) to parties directly related to the conference including sponsors, key suppliers and future conferences. Your name and contact information may be used by the Conference Secretariat and other approved stakeholders, for marketing and managing this and future events of this type.</p>
<h3>Please direct all registration enquiries to:</h3>
<p><b>AMTA2020 Conference Secretariat</b> <br>
    ICMS Australasia <br>
    Jennifer Lynch – Delegate Services <br>
    <a href="mailto:registration@austmtaconference.com.au">registration@austmtaconference.com.au</a></p>


<?php
include('include/footer.php');
?>
